...
function setupEventListeners() {
    ...
    if (rateMainValue) {
        rateMainValue.addEventListener('input', () => {
            updateRateDisplay();
        });
        
        rateMainValue.addEventListener('change', () => {
            updateRateDisplay();
        });
    }
...
function handleTimeComparisonFavorite(event) {
    ...
    if (timeComparisonCountries.includes(countryCode)) {
        // 如果已经是收藏国家，则直接移除
        console.log('Removing from time favorites:', countryCode);
        timeComparisonCountries = timeComparisonCountries.filter(code => code !== countryCode);
        saveTimeFavorites();
        updateTimeComparisonButtons();
        return;
    }
    ...
    // 直接添加到收藏
    console.log('Adding to time favorites:', countryCode);
    timeComparisonCountries.push(countryCode);
    saveTimeFavorites();
    updateTimeComparisonButtons();
}
...
function applyTimeFavorites() {
    ...
    timeComparisonCountries.forEach((countryCode, index) => {
        const select = document.getElementById(`comparison-select-${index + 1}`);
        if (select) {
...
function updateRateDisplay() {
    ...
    // 获取用户输入的基础数值
    const rateMainValue = document.getElementById('rate-main-value');
    const baseAmount = parseFloat(rateMainValue.value) || 1.0000;
        ...
        if (comparisonSelect && comparisonDisplay) {
            ...
            if (currentRates[country.currency]) {
                const rate = currentRates[country.currency];
                        const convertedAmount = (baseAmount * rate).toFixed(4);
...
window.debugRateConversion = function() {
    ...
    const baseAmount = parseFloat(rateMainValue.value) || 1.0;
    console.log('基础数值:', baseAmount);
        ...
        if (select && select.value) {
            ...
            if (country) {
                console.log(`国家 ${i} (${select.value}):`, {
                    ...
                    convertedAmount: currentRates[country.currency] ? (baseAmount * currentRates[country.currency]).toFixed(4) : 'N/A'
                });
            }
        }
    }
